...
PORT = int(os.environ.get("PORT", 8443))  # Render выдаёт порт
HOSTNAME = os.getenv("RENDER_EXTERNAL_HOSTNAME")  # Автоматически задаётся на Render
...
async def handle_rating(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        "Напишите отзыв текстом (и получите подарок!)" if rating >= 4 else "Что пошло не так? Опишите проблему:"
    )
